...
async function buildWatchList(
  ...
  // Count by (label, city, date). Counted as-found regardless of status:
  // closing an item does not mean the pattern did not happen.
  // A separator that cannot occur in a label or a city name. Written as an
  // ESCAPE, never a literal byte: a raw NUL in the source makes git treat
  // this file as binary -- no diff, no blame, and an editor can silently
  // mangle it.
  const SEP = "\u0000";
  const key = (label: string, city: string) => `${label}${SEP}${city}`;
  const byDay = new Map<string, Map<string, number>>();
  const add = (label: string, city: string, date: string) => {
    const m = byDay.get(key(label, city)) ?? new Map<string, number>();
  ...
  for (const [k, perDate] of byDay) {
    ...
    const priors = priorDates
      ...
      .map((d) => perDate.get(d) ?? 0);
    if (priors.length < 2) continue;

    const daysPresent = priors.filter((n) => n >= WATCH_MIN_UNITS).length + (today >= WATCH_MIN_UNITS ? 1 : 0);
    const med = median(priors);

    // Consecutive from today backwards, so "5 days running" is literally true.
    let consecutive = today >= WATCH_MIN_UNITS;
    let streak = consecutive ? 1 : 0;
    for (const n of priors) {
      if (consecutive && n >= WATCH_MIN_UNITS) streak++;
      else break;
    }
    consecutive = streak >= 3;

    // Median, not mean — one register outage must not define the baseline.
    const worsening = med > 0 && today >= 1.5 * med && today - med >= WATCH_MIN_UNITS;
    const cleared = today === 0 && med >= WATCH_MIN_UNITS && daysPresent >= 3;

    if (worsening) items.push({ label, city, days: daysPresent, consecutive, today, median: med, trend: "worsening" });
    else if (cleared) items.push({ label, city, days: daysPresent, consecutive, today, median: med, trend: "cleared" });
    else if (consecutive && today >= WATCH_MIN_UNITS)
      items.push({ label, city, days: streak, consecutive: true, today, median: med, trend: "steady" });
  }
