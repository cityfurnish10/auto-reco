// What the gate recorded on one calendar day — shared by the Activity screen and
// its Excel export, so a download can never disagree with what was on screen.
// (Moved out of app/api/gate/activity/route.ts unchanged in behaviour.)
...
export async function loadActivity(admin: SupabaseClient, opts: ActivityOptions) {
  const { city = null, guardId = null, direction = null, vehicle = null, agent = null } = opts;
...
export type ActivityPayload = Awaited<ReturnType<typeof loadActivity>>;
